...
export default function LongAnswerTextQuestion() {
    ...
    const [content, setContent] = useState({
        ...
        question: '',
        desc: '',
        input: '<textarea placeholder="محل درج پاسخ..."></textarea>'
    })
    ...
    return (
        <>
            ...
            <form className="flex flex-col gap-4 py-4">
                <InputField max={limits.max} min={limits.min} setLimits={setLimits} onChange={() => setContent(prev => ({ ...prev, input: `<textarea type='text' minLength={${limits.min}} maxLength={${limits.max}}></textarea>` }))} type={'limit'} />
            </form >
            <ContentEditor
                onOpen={() => {
            ...
            <SubmitQBtns
                condition={((content.input !== '<textarea placeholder="محل درج پاسخ..."></textarea>' && content.input !== `<textarea type='text' minLength={${limits.min}} maxLength={${limits.max}}></textarea>`) && content.question && (!shownSections.img || (shownSections.img && (content.img || (content.video && content.video !== '<video src=></video>')))) && (!shownSections.desc || (shownSections.desc && content.desc)))}
                onClick={() => {
